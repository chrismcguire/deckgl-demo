...
const MAPBOX_ACCESS_TOKEN = 'pk.eyJ1IjoiY21jZ3VpcmUiLCJhIjoiY2p2cG5haGxoMHNkZzRibzQ3bmNoeGFuNCJ9.rp8dluzL12JOOANArXFjjQ';
...
const lineData = [
	{sourcePosition: [-75.163700, 39.950220], targetPosition: [-75.163700, 39.956220]},
	{sourcePosition: [-75.163700, 39.950220], targetPosition: [-75.163919, 39.950606]},
	{sourcePosition: [-75.163700, 39.950220], targetPosition: [-75.163481, 39.950606]},
];
...
class BikeMap extends React.Component {
	...
	componentDidMount() {
		const data = fetchJSON('https://raw.githubusercontent.com/chrismcguire/deckgl-demo/master/data/Bike_Network.geojson')
...
class SimpleZip extends React.Component {
	...
	componentDidMount() {
		const data = fetchJSON('https://raw.githubusercontent.com/chrismcguire/deckgl-demo/master/data/zip-trees.geojson')
            .then(function(data) { return data });
	  this.setState({
			zips: data,
		});
...
class StyledZipMap extends React.Component {
	...
	componentDidMount() {
		const data = fetchJSON('https://raw.githubusercontent.com/chrismcguire/deckgl-demo/master/data/trees_in_zip.geojson')
            .then(function(data) { return data });
...
class ZipTreeMap extends React.Component {
	constructor(props) {
    super(props);
		this._onMapLoad = this._onMapLoad.bind(this);
	...
	componentDidMount() {
		const data = fetchJSON('https://raw.githubusercontent.com/chrismcguire/deckgl-demo/master/data/zip-trees.geojson')
            .then(function(data) { return data });
		const paths = fetchJSON('https://raw.githubusercontent.com/chrismcguire/deckgl-demo/master/data/PPR_StreetTrees.geojson')
            .then(function(data) { return data });
	  this.setState({
			zips: data,
			paths: paths
...
function App() {
  return (
    <Router>
      <Route exact path="/simple" component={SimpleApp} />
      ...
      <Route exact path="/zip-map" component={ZipTreeMap} />
    </Router>
  );
}
